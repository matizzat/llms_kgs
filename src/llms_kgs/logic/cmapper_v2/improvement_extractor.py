...
class ImprovementExtractor:
    """Improves a triple list from a given focus question and chunk."""
    ...
    def __init__(self,
        ...
        """Important precondition: user_template has fields {text}, {triples} and {focus_question}."""
    ...
    def extract(self, creation_state: CMapCreationState):
        ...
        # Create a string from current triple list:
        triples_as_string = "\n".join([t.source.label + " @ " + t.relation.label + " @ " + t.target.label
                                       for t in creation_state.triples_refinements[-1]])
